...
def calculate_checksum(input_string: str) -> int:
    return sum((i + 1) * ord(char) for i, char in enumerate(input_string)) + len(
        input_string
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Calculate a checksum for a given string."
    )
    parser.add_argument(
        "input_string", type=str, help="The string to calculate the checksum for."
    )
    ...
    # for i in alphabet:
    #
    #     result_test = pad_repeat_until(calculate_checksum(f"{i}{j}{k}"), 12)
    #     if result_test == result:
    #         print(f"ahah found : {i}{j}{k} with same hash")
    #         break
    # print("nothing found")
